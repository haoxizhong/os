...
public class StubFileSystem implements FileSystem {
	...
	try {
	    return new StubOpenFile(new File(directory, name), create);
	}
	catch (IOException e) {
	    return null;
	}
	...
	StubOpenFile(final File f, final boolean create) throws IOException {
	    super();
		...
		public void run() { getRandomAccessFile(f, create); }
	    });
	...
	private void getRandomAccessFile(File f, boolean create) {
	    try {
		if (create || f.exists())
